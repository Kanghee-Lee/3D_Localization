...
def collate_pair_fcgf(list_data):
  ...
  # print('trans : ', trans)
  fcgf_batch0, fcgf_batch1 = [], []
  xyz_batch0, xyz_batch1 = [], []
  matching_inds_batch, trans_batch, len_batch0, len_batch1 = [], [], [], []
  ...
  for batch_id, _ in enumerate(fcgf0):
    N0 = fcgf0[batch_id].shape[0]
    N1 = fcgf1[batch_id].shape[0]
    ...
    fcgf_batch0.append(to_tensor(fcgf0[batch_id]))
    fcgf_batch1.append(to_tensor(fcgf1[batch_id]))
  ...
  fcgf_batch0 = torch.cat(fcgf_batch0, 0).float()
  fcgf_batch1 = torch.cat(fcgf_batch1, 0).float()
...
class IndoorPairDataset(PairDataset):
  ...
  def __getitem__(self, idx):
    if self.fcgf_extract :
      file0 = os.path.join(self.root, self.files[idx])
      data0 = np.load(file0)
      xyz0 = data0["pcd"]
      color0 = data0["color"]
      ...
      return (self.files[idx][0], self.files[idx][1], xyz0, xyz1, coords0, coords1, feats0, feats1, trans, pcd_match)
