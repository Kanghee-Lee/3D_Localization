...
def best_pos_distance(query, pos_vecs):
    with tf.name_scope('best_pos_distance') as scope:
        ...
        print('squared diff : ', tf.squared_difference(pos_vecs, query_copies))
        print('reduc sum diff : ', tf.reduce_sum(tf.squared_difference(pos_vecs, query_copies), 2))

        best_pos = tf.reduce_min(tf.reduce_sum(tf.squared_difference(pos_vecs, query_copies), 2), 1)
        print('reduc min diff : ', best_pos)
        # assert 0
...
def lazy_quadruplet_loss(global_descs, batch_size, num_pos, num_neg, global_triplet_margin=0.5, global_quadruplet_margin=0.2, **kwargs):
    ...
    qvec, posvec, negvec, othernegvec = tf.split(global_descs,
                                                 ...
                                                 axis=0)
    print('loss - qvec : ', qvec.shape)   # (2, 256)
    print('loss - posvec : ', posvec.shape) # (4, 256)
    print('loss - negvec : ', negvec.shape) # (16 ,256)
    print('loss - othernegvec : ', othernegvec.shape) # (2, 256)

    q_vec = tf.reshape(qvec, [batch_size, 1, outdim], name='qvec')  # (2, 1, 256)
    pos_vecs = tf.reshape(posvec, [batch_size, num_pos, outdim], name='posvec') # (2, 2, 256)
    neg_vecs = tf.reshape(negvec, [batch_size, num_neg, outdim], name='negvec') # (2, 8, 256)
    other_neg_vec = tf.reshape(othernegvec, [batch_size, 1, outdim], name='othernegvec')    # (2, 1, 256)
