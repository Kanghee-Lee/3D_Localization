...
class Global_test_dataset(RNGDataFlow):
    def __init__(self, basedir, test_file,
        ...
        self.eval_list = [self.eval_list[i * 12:(i + 1) * 12] for i in range((len(self.eval_list) + 12 - 1) // 12)]
        self.eval_list[-1].append(self.eval_list[-1][-1])
    ...
    def get_pcd_list(self):
        ...
        for seq in self.eval_sequences:
            ...
            for pcd in seqinfo:
                pcdlist.append(pcd['query'] + '.npz')
        print("{} pointclouds to predict. \n".format(len(pcdlist)))
        return pcdlist

    def __iter__(self):
        for i in range(self.size):
            names = self.eval_list[i]
            ...
            #     pcs.append(pc)
            for name in names:
                fcgf_pcfile = os.path.join(self.basedir, name)
                fcgf = np.load(fcgf_pcfile)
                lengths.append(fcgf['feature'].shape[0])
                fcgfs.append(fcgf['feature'])

            fcgfs = np.concatenate(fcgfs)

            yield [fcgfs, np.array(lengths).reshape(-1, 1), names]
