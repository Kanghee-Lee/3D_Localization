...
class DH3D(ModelDesc):
    ...
    def inputs(self):
        ...
        ret.append(
            tf.TensorSpec((self.config.batch_size, None, 32), tf.float32,
                          'anchor_feat'))
        ret.append(
            tf.TensorSpec((self.config.batch_size, 1), tf.float32,
                          'anchor_len'))
        if self.config.num_pos > 0:
            ...
            ret.append(
                tf.TensorSpec((self.config.batch_size, None, 32), tf.float32,
                              'pos_feat'))
            ret.append(
                tf.TensorSpec((self.config.batch_size, 2), tf.float32,
                              'pos_len'))
        if self.config.num_neg > 0:
            ...
            ret.append(
                tf.TensorSpec((self.config.batch_size, None, 32), tf.float32,
                              'neg_feat'))
            ret.append(
                tf.TensorSpec((self.config.batch_size, 8), tf.float32,
                              'neg_len'))
    ...
    def compute_global(self, outs, freeze_global=False):
        with backbone_scope(freeze=freeze_global):
            ...
            print('forglobal shape : ', forglobal.shape)   # (24, 8192, 256)
            ## if sample
            ...
            print('global_att shape : ', global_att.shape)   # (24, 8192, 1)
            assert 0
            inputs_dict = {
                'xyz': newpoints,
                'features': forglobal,
...
############################################################ FCGF
    def build_graph(self, *inputs_dict):
        ...
        print('out - global : ', outs['global_desc'].shape)   # (24, 256)
