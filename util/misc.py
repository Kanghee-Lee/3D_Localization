...
def extract_features(model,
  ...
  return return_coords, model(stensor, stensor.shape[0]), stensor.shape[0]


#################################################################

def extract_globalDesc(model,
                     xyz,
                     rgb=None,
                     normal=None,
                     voxel_size=0.05,
  ...
  return return_coords, model(stensor)

#################################################################

def extract_globalDesc_from_fcgf(model,
                     fcgf,
                     length,
                     device=None,
                     is_eval=True):
  ...
  if device is None:
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
  fcgf = torch.from_numpy(fcgf)
  fcgf = fcgf.to(device)
  return model(fcgf, length)